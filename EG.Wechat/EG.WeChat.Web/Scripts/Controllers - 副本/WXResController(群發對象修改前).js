...
var WXResController = angular.module("WXResController", ['WXService', 'WXDirective']);

//图片资源列表Controller
//mainApp.controller("PictureListCtrl", function ($scope, $http, instance) {
WXResController.controller('PictureListCtrl', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
    //
    var url = "/WXResource/LoadWXPictures";
    $http.post(url).success(function (response) {
        $scope.pictures = response.ListJson;
        instance.pictures = response.ListJson;
    });
    //"/Images/WXResources/635545229404488369.jpg"
...
//图文资源列表Controller
//mainApp.controller("ArticleListCtrl", function ($scope, $http, instance, ptser) {
WXResController.controller('ArticleListCtrl', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
    var url = "/WXResource/LoadWXArticles";
    $http.post(url).success(function (response) {
    ...
    $scope.Delete = function (PTargetEditIndex) {
...
//視頻资源列表Controller
WXResController.controller('VideoListCtrl', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
    var url = "/WXResource/LoadWXVideos";
    $scope.videos = null;
    ...
    $scope.DeleteVideo = function (PTargetEditIndex) {
...
//音頻資源列表Controller
WXResController.controller('VoiceListCtrl', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
    var url = "/WXResource/LoadWXVoices";
    $scope.voices = null;
    ...
    $scope.DeleteVoice = function (PTargetEditIndex) {
...
//群发消息Controller
//mainApp.controller("GroupSendCtrl", function ($scope, instance, ptser) {
WXResController.controller('GroupSendCtrl', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
    ...
    $scope.CreateGroupMessage = function () {
        ...
        $.ajax({
            ...
            data: { "mediaid": $scope.curMediaID, "textcontent": $scope.text, "msgtype": $scope.msgType },
            type: "post",
            dataType: "json",
            success: function (data) {
...
//群发消息Controller
//mainApp.controller("GroupSendReviewCtrl", function ($scope, $http, instance, ptser) {
WXResController.controller('GroupSendReviewCtrl', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
    ...
    $scope.ChangeCur = function (CurGs) {
        ...
        if (CurGs.ContentType == 'text') {
            ...
            angular.forEach(instance.Articles, function (item) {
                if (item.media_id == CurGs.SContent) {
                    $scope.article = item;
                }
            });
    ...
    $scope.SendGroupMessage = function () {
        ...
        if ($scope.sendType == 1) {
            ...
            $.ajax({
                ...
                data: { "messageid": $scope.CurGSMessage.ID, "mediaid": $scope.curMediaID, "textcontent": $scope.text, "msgtype": $scope.CurCType, "groupid": $scope.CurGroupId },
                type: "post",
                dataType: "json",
                success: function (data) {
                    if (data.IsSuccess == false)
    ...
    //切换选中分组
    $scope.ChangeSelectGroup = function (id) {
        $scope.CurGroupId = id;
    }
    //消息类型下拉框默认标题
