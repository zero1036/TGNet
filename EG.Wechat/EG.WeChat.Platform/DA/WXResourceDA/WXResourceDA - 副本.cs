...
namespace EG.WeChat.Platform.DA
{
    public class WXResourceDA : WXResAOP
    {
        ...
        /// <returns></returns>
        public override int? SaveResource(int? lcid, string lcname, string lcclassify, string media_Id, string media_Type, string content, DateTime iCreateTime)
        {
            DataSet result = template.ExecuteX(m_proceName_UpdateRESOURCE, new string[] { "@Plcid", "@Plcname", "@Plcclassify", "@Pmediaid", "@Pmediatype", "@Pcontent", "@Pctime" }, new object[] { lcid, lcname, lcclassify, media_Id, media_Type, content, iCreateTime }, null, CommandType.StoredProcedure);
